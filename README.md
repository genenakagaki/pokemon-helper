...
* [`src/pokemon_helper/`](src/pokemon_helper/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/pokemon_helper/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `pokemon-helper` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/pokemon_helper/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns pokemon-helper.example
  (:require [pokemon-helper.config :as config])
